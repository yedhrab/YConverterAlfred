...
def main():
    amount = float(argv[1])
    if len(argv) == 2:
        AlfredResponse.write("Waiting for amount...", f"{amount} source destination", icon_path="fetching.png")
        return

    source = argv[2].upper()
    if len(argv) == 3:
        AlfredResponse.write("Waiting for source symbol...", f"{amount} {source} destination", icon_path="fetching.png")
        return
    ...
    if not 3 <= len(destination) <= 4:
        AlfredResponse.write(
            "Waiting for destination symbol...", f"{amount} {source} {destination}", icon_path="fetching.png"
        )
        return
